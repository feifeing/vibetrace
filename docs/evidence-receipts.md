...
VibeTrace Evidence Receipts are deterministic integrity records for completed checkpoints. They are designed to answer a narrow question:
...
Evidence Receipts use the `vtr_*` prefix:

```text
vtr_<24 hex characters>
```

The identifier is derived from a stable SHA-256 projection of versioned checkpoint evidence.

`vibetrace verify` recomputes the receipt for the stored receipt's own evidence version. This matters because VibeTrace intentionally preserves the verification semantics of older checkpoints instead of silently reinterpreting them under a newer receipt schema.
...
That did not make the Git checkpoint objects invalid evidence. In fact, `vibetrace contract-delta` was deliberately designed to recompute its observed effect from the before/after Git objects instead of trusting `analysis.files`.
...
VibeTrace does not silently replace a stored v1 receipt with a v2 receipt during verification, because doing so would change the historical receipt identifier and pretend the original receipt covered evidence that it never covered.
...
When VibeTrace saves a completed checkpoint that already has a supported Evidence Receipt, it preserves the stored receipt's evidence version.

This rule prevents an unrelated resave from silently turning:
...
New checkpoints with no existing receipt use the current receipt version.
...
Receipt verification is one layer of `vibetrace verify`.

The full command also checks, when available:

- whether before/after Git commit objects still exist;
- whether VibeTrace private refs still point at the recorded objects; and
